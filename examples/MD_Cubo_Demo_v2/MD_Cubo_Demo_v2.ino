// RM version, Works good. Versie 4.  26 aug 2019   (Ruud van der Meer The Netherlands.)
// Changes:
// Pullup on A6, switch to GND, normal random, by sw to gnd during startup, random is OFF!
// On A7 a long wire for the randomfunction....
// Final version for this moment....
...
// void CornerToCorner() is change to work good!
...
MD_Cubo_JC  C;
#endif
#ifdef MD_CUBO_8x8_ICS574_H
MD_Cubo_ICS574  C;
#endif

bool RANDOM_CYCLE = 1; // 1 for random selection, 0 for sequential cycle: Default random

// Debug not possible with the ICS574 hardware!
...
#define DEMO_RUNTIME  15000  // in miliseconde  NOW 15 seconds for each pattern.
uint32_t timeStart;
...
void recedingText()
{
  const char  message[] = "= Arduino training bij de LVU. ";
...
void scrollingText()
{
  ...
  char message[] = "LVU         Arduino           Kubus.";
  // this only works for larger cubes, so return if the cube is not big enough
  if ((C.size(MD_Cubo::XAXIS) < 8 || C.size(MD_Cubo::YAXIS) < 8 || C.size(MD_Cubo::ZAXIS) < 8))
    return;
...
void CornerToCorner()
{
  ...
  while (millis() - timeStart <= DEMO_RUNTIME){
      ...
      whatstep++; 
      if(whatstep > (C.size(MD_Cubo::XAXIS)-1)*3) {whatstep = 0; growing = false;}
    }
    else {
      for(uint8_t x = 0 ; x < C.size(MD_Cubo::XAXIS) ; x++)
      {
...
#endif
  C.begin();
  randomSeed(analogRead(A7));  // sort of gives different start results.
  C.clear(VOX_ON); // All leds ON
  delay(1000);
  C.clear(VOX_OFF); // All leds OFF
  delay(500);
  // by startup look to the switch Switch with pullup resist to +5V
  // switch to GND = sequential
